...
export const options = {
  scenarios: {
    ingest: {
      ...
      rate: Math.ceil(entriesPerSecond / batchSize),
      timeUnit: "1s",
...
function makeLog(i) {
  const marker = `${__VU}-${__ITER}-${i}-${Date.now()}`;
  return {
    timestamp: new Date().toISOString(),
    level: i % 20 ? "info" : "error",
    service: i % 3 ? "checkout" : "api",
    message: `mixed payment marker-${marker}`,
    attributes: { marker, region: i % 2 ? "eu" : "us", retries: i % 4, cached: i % 2 === 0 },
  };
}

export default function ingest() {
  const logs = Array.from({ length: batchSize }, (_, i) => makeLog(i));
  attempted.add(logs.length);
...
export function aggregate() {
  ...
  const response = http.get(
    ...
    { headers: { Authorization: "Bearer k6" }, tags: { route: "aggregate" } },
  );
